...
function get_permalink($type, $slug = '', $arrs = []){
	...
	if(PRETTY_URL && $slug){
		...
		if(isset($options['lang_code_in_url']) && filter_var($options['lang_code_in_url'], FILTER_VALIDATE_BOOLEAN)){
			global $lang_code;
			if(isset($lang_code)){
				$lang_id = $lang_code.'/';
			}
	...
	if($type === 'index'){
		...
		if(isset($_GET['lang'])){
			...
			if(strlen($_GET['lang']) <= 3){
				setcookie('lang', $_GET['lang'], strtotime('+4 months'), '/');
				$lang = $_GET['lang'];
			}
		}
