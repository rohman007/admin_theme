		...
		$.ajax({
			...
			complete: function (data) {
				...
				if(res['update']){
					...
					if(!only_check){
						$('.update-info').append('<div class="alert alert-info alert-dismissible fade show" role="alert">New update is available! CloudArcade v'+res['update']+' , open "Updater" for more info!<button type="button" class="btn-close text-white" data-bs-dismiss="alert" aria-label="Close"></button></div>');
					}
				} else {
